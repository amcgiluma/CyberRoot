...
- `[HECHO]` (23/08) **Gate de Juanma — APROBADO el 26/08 23:25** ("OK ACTIVA EL CONCILIO"). Pyxel ratificado como stack. ↩ (26/08) Concilio activado: 9 agentes `scheduled`, primer turno 27/08 03:00 (Manus). *(archivar a `hecho/2026-08.md`)*
...
- `[PENDIENTE][P1]` (27/08) **Tutorial cap. 0: enseñar a copiar antes/igual que pedirlo** — Oscar: el dossier pide «copiar» pero la escena técnica (Manus, `CAPITULOS/00-la-firma.md`) solo muestra `ls`/`cat`/`cd ..`; `cp` no entra hasta el cap. 1 (§6.2) mientras el cap. 0 declara 3 conceptos (§6.3) y la extracción de datos depende de copiar (§7.1). Un jugador de cero recibe un objetivo que exige un comando no enseñado (viola la directriz Bandit aprender-por-necesidad de DESIGN §6). Decidir (Gwyn, dirección): o `cp` entra como 4.º concepto del cap. 0, o el objetivo inicial se reformula. No implementar todavía (sin build); consignar en el plan del día cuando toque.
- `[PENDIENTE][P2]` (27/08) **Definir si la run 0 puede fallar o no** — Oscar: §6.1 la describe como run guiada «todo sale bien», pero el capítulo materializado incluye bloque «si mueres en la primera run» con post-mortem de expulsión al 40 %. Si no puede fallar, rama muerta; si puede, contradice el beat 1 y agrava el hallazgo de `cp`. Gwyndolin: cerrar el comportamiento de la run 0 en el diseño (decisión de Gwyn).
