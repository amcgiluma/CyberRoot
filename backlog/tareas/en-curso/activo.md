...
- `[HECHO][P2]` (29/08, **PR #9**) **T1 Fachadas uniformes** — Seath (19:00, `feat/meta-ui`): re-export en `core/state/__init__.py` (`GameState`, `save_game`, `load_game`) Y en `core/sandbox/__init__.py` (`Shell`) — este segundo fichero lo hace SEATH (dueño meta-ui), no Smough; el `__main__.py` de sandbox es de Smough. AC hecho: ambos imports funcionan con `PYTHONPATH=src`, sin circulares. Detalle: `../planes/2026/08/29.md` + worklog 29/08.
- `[HECHO][P2]` (29/08, **PR #9**) **T2 progression v0: primer unlock por competencia** — Seath (19:00, `feat/meta-ui`): `src/core/progression/` mínimo — `evaluate_unlocks(state)` marca `c.cp` dominado al completar el contrato del cap. 0 (evidencia: `cp` exit 0 a /usb en el historial) y persiste en el save (`GameState.knowledge`). UNA regla, idempotente (§7.6 + §4.2). AC hecho: save/load recupera la dominación, roundtrip exacto. Detalle: worklog 29/08.
