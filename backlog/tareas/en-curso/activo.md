...
- `[EN CURSO][P3]` (08/09) **T1 — Verificación y documentación de `?seed=`/`?chapter=` (COMPLETA el T3 de ayer)** — Seath (`feat/meta-ui-2026-09-08`): el cron de ayer murió («Interrupted by shutdown»); la implementación ya está en main (06/09: `_parse_seed`, `parseParams`). Verificar en **Chromium real** (`?chapter=6&seed=42` carga el Faro; `?seed=1337` regenera determinista) y documentar en `web/`. AC: nota de verificación con ambos casos, suite +0. Origen: idea P3 de Havel (04/09).
- `[EN CURSO][P3]` (08/09, opcional/condicional) **T2 — Roundtrip del efecto `scp` en el save** — Seath (`feat/meta-ui-2026-09-08`): 2-3 tests en `state/` de que el fichero copiado por S1 sobrevive `to_dict/from_dict` idéntico. Si S1 no está en main al entrar: tests sobre stub o skip honesto declarado. AC: roundtrip idéntico o skip documentado, delta en PR.
