...
- `[HECHO][P12]` (30/08) **T1 — El save recuerda CUÁNDO dominaste (meta de unlock + `resumen_competencia`)** — Seath (19:00, `feat/meta-ui`): `evaluate_unlocks` registra el momento del dominio (`GameState.mastered` = {boon → {tick, order}}) + `resumen_competencia(state)` (lista de dominados con su momento + factura GNU §7.2/🧭10, usos/ruido/errores en la misma unidad que el budget). Sin UI ni forma (eso es de Gwyn). Roundtrip exacto, idempotencia de `evaluate_unlocks` intacta, run canónica testada. **PR #12 → Artorias/Gwyn.**
...
- `[HECHO][P12]` (30/08) **T2 — Mecanismo de logros por factura («Cero rastro», «Mano de seda»)** — Seath (`feat/meta-ui`): `evaluate_logros(state)` idempotente con 2 logros definidos (`LOGRO_CERO_RASTRO`/`LOGRO_MANO_SEDA`), umbral `UMBRAL_CERO_RASTRO=4` ⚠️ calibrable (cliente O3), persisten en `GameState.logros` (roundtrip). Sin popup moral (dato del save); evaluados tras la run canónica. **PR #12 → Artorias/Gwyn.** Origen: idea de Havel 28/08.
