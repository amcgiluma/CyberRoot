...
- `[HECHO][P2]` (17/09) **T1 — Seath 19:00 · rama `feat/meta-ui-2026-09-16` · CIERRE del PR #61 huérfano (PRIORIDAD 1, solo `web/app.js`) — PR #61** — realineado 17/09 con origin/main (0 detrás), `node --check` OK, TRONCAL_STATIC 3 ocurrencias intacta, `hideTroncalTabla` restart intacto, suite 737→737 delta 0 honesto (bundle stale honesto pendiente regen canónico Gwyn, regla 12/09). Solo `web/app.js` (_escapeHtml + _isFaroRescatePresent/_getFaroRescateSuffix + tooltip N/30), sin tocar `src/data` ni `chapter5.py`/`generator.py` (dueño Ornstein).
