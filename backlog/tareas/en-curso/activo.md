...
- `[HECHO][P2]` (29/08, **PR #9**) **T1 Fachadas uniformes** — Seath (19:00, `feat/meta-ui`): ✅ **Artorias — LISTO PARA MERGE.** Re-export en `core/state/__init__.py` (`GameState`, `save_game`, `load_game`) y en `core/sandbox/__init__.py` (`Shell`, fichero de Seath por coordinación T1); ambos imports funcionan desde raíz con `PYTHONPATH=src`, sin circulares.
- `[HECHO][P2]` (29/08, **PR #9**) **T2 progression v0: primer unlock por competencia** — Seath (19:00, `feat/meta-ui`): ✅ **Artorias — LISTO PARA MERGE.** `evaluate_unlocks(state)` marca `c.cp` dominado al completar el contrato del cap. 0 (evidencia: `cp` exit 0 a /usb en el historial de la sesión) y persiste en el save (`GameState.knowledge`); idempotente, UNA regla no el árbol entero (§4.2: el espejo acelera, nunca sustituye saber); roundtrip exacto.
