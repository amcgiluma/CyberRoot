...
- `[EN CURSO][P2]` (16/09) **🧭36 — O1 — Ornstein 13:00 · `feat/engine-2026-09-16` · `session.py`: e3 expone la disolución por `rm` (simetría scp/rm en el flujo de encargo)** — `_commands_for(4)` devuelve `DEFAULT_CH4E3_COMMANDS` (14, con `rm`) cuando contract_id/quest_id == 'story.ch4.e3'; base ch4 13 INTACTA en e1/e2. Sin tocar `shell.py` ni asserts de allowlist (allowlist e3 ya existe del PR #57). Criterio: `abrir_encargo(c,'story.ch4.e3')` → `rm /tmp/volcado.csv` exit 0 → post-mortem `volcado: caducado` por `rm`; e1/e2 siguen base 13 (rm→127, frontera intacta). Tests FLEXIBLES e1/e2↔e3. Suite 727→731 (+4). **ALLOWLIST OWNER: NADIE hoy.**
- `[EN CURSO][P2]` (16/09) **S1 — Smough 16:00 · `feat/sandbox-2026-09-16` · `dato7` «El fantasma que pesa» — el volcado que viaja al Faro** — `src/core/generator/`: el FS del Faro planta `/srv/camara-faro/volcado-rescate.csv` con `TR-003|EN_COLA` SOLO si la sesión rescató (detector `_has_volcado_rescate` ya existente; si caducado → fichero NO existe y el join nombra la ausencia); + `src/data/curriculum.json` quest `story.ch6.dato7` grey requires `c.join` — golden `join -t'|' -1 1 -2 3 -v 1 purgas.csv volcado-rescate.csv | grep TR-003` → exit 0. Gate 24/30→31. Primera consecuencia cruzada de capítulos del juego. Suite 727→734. **GATE OWNER de dato7: Smough.** Bundle lo regenera Smough toca curriculum (regla 12/09).
- `[EN CURSO][P2]` (16/09) **T1 — Seath 19:00 · `feat/meta-ui-2026-09-16` · Web: tooltip del `N/30` + panel rescate en el Faro (solo `web/app.js`)** — (a) tooltip sobre `· ticks del volcado: N/30` (`title=` descriptivo, consume `get_history()` ya expuesto, sin pulso — 🧭34 respetado); (b) lente del rescate: el panel del Faro detecta `volcado-rescate` y anota `TR-003` (lente pura, NADA ejecuta el core). Sin tocar `TRONCAL_STATIC` (deuda vigente), sin tocar `src/`, NO regenera bundle. Suite delta 0 honesto.
