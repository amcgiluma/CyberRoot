...
- `[EN CURSO][P2]` (30/08) **S1 — Tuberías en el sandbox + `grep`/`wc` (cap. 2)** — Smough (16:00, `feat/sandbox`): una tubería `cmd1 | cmd2`, rechazo didáctico `&&`/`;` se mantiene, ruido por comando de la tubería. AC: `grep 11:04 centralita/turnos/turno.log | wc -l` (línea EXACTA del cap. 2) golden contra GNU real. **[HECHO → PR #11]** (Smough 30/08): tubería soportada (stdout→stdin, UNA pipe; `a|b|c` rechazo propio), `commands/texto.py` con `grep`/`wc` GNU, perfil grep:2/wc:1, runner con `stdin`; `test_texto.py` + `test_session_ch2.py` (línea exacta → `2`) + cap. 0 intacto. Suite 360 passed + 1 xfail (el de costura de Ornstein, no mío).
- `[EN CURSO][P2]` (30/08) **S2 — `story.ch2.*` al currículo (cap. 2 «Facturas» de Manus)** — Smough (16:00, `feat/sandbox` + `src/data/`): 5 quests (`e1`–`e5`, 2 azules/1 gris/1 rojo/1 cierre) + conceptos mínimos (`c.grep`/`c.wc`/`c.pipe`). AC: `load_curriculum` + validador en negativo. **[HECHO → PR #11]** (Smough 30/08): `curriculum.json` con `c.grep`→`c.wc`→`c.pipe` (familia texto, cap. 2) + `story.ch2.e1`–`e5` (tints blue/blue/grey/red/grey según Manus); `load_curriculum` carga (14 conceptos, 11 quests) y el validador pasa en positivo y negativo (sin ciclos ni prereqs fantasma). `test_loader.py` con cobertura de requires/tints del cap. 2.
