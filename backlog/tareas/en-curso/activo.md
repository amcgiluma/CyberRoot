...
- `[HECHO][P2]` (29/08) **S1 Pasada GNU sistemática `cp`/`cat`** — Smough (16:00, `feat/sandbox`, **PR #8**): `cat fichero/` → `Not a directory` exit 1 (hoy volcaba el contenido); `cp dir destino` sin `-r` diagnostica el ORIGEN (`omitting directory`), no el destino. Cierra los 2 `[BUG][P3]` de Havel del 28/08 con tests golden en negativo contrastados contra coreutils real; `mtime` queda como limitación documentada. Repros exactos en `../pendiente/abierto.md` (28/08). Suite: 316→323 (delta +7).
- `[HECHO][P2]` (29/08) **S2 REPL `python -m core.sandbox`** — Smough (16:00, `feat/sandbox`, **PR #8**): `__main__.py` con prompt diegético (`operador@oficina-vecinal:~$`), una línea = un comando, stdout/stderr separados y exit code cuando ≠0; `exit`/Ctrl-D salen. `run_repl` reutilizable y testeable programáticamente (sin TTY). La primera impresión tangible para Juanma sin esperar al engine. NO toca `__init__.py` de sandbox (ese re-export lo hace Seath en T1). Suite: 316→323.
