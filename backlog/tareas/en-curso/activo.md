...
- `[HECHO][P3]` (25/09) **S1 — Smough 16:00 · `feat/sandbox-2026-09-25` · 🧭47: contraste stock de Gris + lecturas intercaladas** — ✅ PR #82 (Smough) — corpus `--stock-gris` que mide vía `_probar_micro_real` real la distribución N=8 de azul vs rojo (20→90.0% / 500→99.6% pct>=3, K 8→4 intercalado delta 0.0% pct) + lectura `grep censo` (0 gris) intercalada entre `chmod 600` (mismo N=8: no cruza T=3, solo diluye densidad) — stock Gris 0% estático medido honesto (§8.6), si +0.3 decide Gwyn. `pytest == 818` (+0, 818+deltaO1 en main), `python tools/harness/run_seeds.py --stock-gris --karma-seeds 20 --export /tmp/stock.json` verde (hist 100% determinista), payload `stock_gris` exportado, README harness con cifras 25/09. Solo `tools/harness/run_seeds.py` + `tools/harness/README.md`. plan `../planes/2026/09/25.md` §S1 con AC.
