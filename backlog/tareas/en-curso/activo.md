...
- `[HECHO][P3]` (24/09, 🧭45) **S1 — Smough 16:00 · `feat/sandbox-2026-09-24` · calibración micro-karma a 20 runs (pesos antes que prosa)** — `tools/harness/run_seeds.py` modo `--micro-karma` (ancla real 6/6, corpus 20×3 N=8, ventana K, contraste y proyección weight 2) — **PR #TBD** (809 passed, delta +0, ancla {chmod600:1, chmod777:-1, chown_gris:1, chown_root:-1, hup:1, kill:-1}, 90% ≥3 en 3 runs, 95% con weight 2 en 2 runs) — AC: N=20 corre sin error; tabla en `docs/worklog/2026/09/24.md` + 🎯 Smough en `notas-manana.md`; SIN tocar `src/` ni `web/` ni data
