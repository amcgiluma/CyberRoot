...
- `[EN CURSO][P3]` (06/09, REPOSICIÓN 08/09) **O1 — Tests `auditor_orden` (+4)** — Ornstein (`feat/engine-2026-09-08`): `src/tests/core/engine/test_auditor_orden.py`, los 4 casos de la línea de Artorias; SOLO tests, `postmortem.py`/`textos.json` intactos. AC: 617→621, delta declarado. Ayer NO ENTREGADO (💥 en sección 07/09): sin rama/PR en remoto. **PRIORIDAD 1 de hoy (Gwyn); orden O1→O2→O3.**
- `[EN CURSO][P1]` (08/09, REPOSICIÓN con clave corregida) **O2 — Quest `story.ch6.e2` «La que no pesa» (cap. 6)** — Ornstein (`feat/engine-2026-09-08`): ⚠️ **NUNCA `story.ch6.e1`** — ya es «El número que sobra» (02/09); la quest nueva nace como **`story.ch6.e2`** (hallazgo de Gwyndolin 08/09). Quest + FICHA vacía + golden con `tail -n +2` (🧭22) + briefing «un distrito se repite» (🧭23) + columna `marcas_purga` + textos `story.ch6.e2.*` + validador. AC: gate 23→24 quests, `e1` intacta, goldens dato2/dato3 exit 0, suite 621→625.
- `[EN CURSO][P2]` (08/09, REPOSICIÓN) **O3 — `/etc/hosts` en el mundo (host `faro`)** — Ornstein (`feat/engine-2026-09-08`): generator escribe `/etc/hosts`; AC: `cat /etc/hosts` exit 0 en `generate(42,6)` y descubrimiento de `faro` si el stack lo permite (`_note_hosts_discovery` opera en `cat` exit 0); determinismo byte-idéntico salvo el fichero; suite 625→627. Costura O↔S: la Fase A pasa de stub a mundo real.
- `[EN CURSO][P2]` (08/09) **S1 — Handler `scp` Fase B (red pieza 2, primera pieza del cap. 4)** — Smough (`feat/sandbox-2026-09-08`): copia entre FS del stack de conexión (`scp [user@]host:ruta destino`); host no descubierto → rechazo que NOMBRA qué falta y dónde (`/etc/hosts`), exit 1, ruido 0. AC: 4 tests (copia ok, rechazo no-descubierto, ruta remota inexistente, destino inválido), 627→631. Origen: red simulada P2 (31/08) — ELEGIDA HOY (solo handler; quests `ch4` mañana).
- `[HECHO][P3]` (08/09) **T1 — Verificación y documentación de `?seed=`/`?chapter=` (COMPLETA el T3 de ayer)** — Seath (`feat/meta-ui-2026-09-08`): el cron de ayer murió («Interrupted by shutdown»); la implementación ya está en main (06/09: `_parse_seed`, `parseParams`). Verificar en **Chromium real** (`?chapter=6&seed=42` carga el Faro; `?seed=1337` regenera determinista) y documentar en `web/`. AC: nota de verificación con ambos casos, suite +0. **PR #38 pendiente (rama `feat/meta-ui-2026-09-08`). Verificado Chromium headless shell 153: `?chapter=6&seed=42` → hint-cap6 block + md 6/42 pool `c.cut…c.uniq`/budget 12; `?seed=1337` → cap0/1337 determinista reload. Doc en `web/README.md` §Verificación.** Origen: idea P3 de Havel (04/09).
- `[HECHO][P3]` (08/09, opcional/condicional) **T2 — Roundtrip del efecto `scp` en el save** — Seath (`feat/meta-ui-2026-09-08`): 2-3 tests en `state/` de que el fichero copiado por S1 sobrevive `to_dict/from_dict` idéntico. Si S1 no está en main al entrar: tests sobre stub o skip honesto declarado. AC: roundtrip idéntico o skip documentado, delta en PR. **3 tests en `src/tests/core/state/test_state_scp_roundtrip.py` — stub honesto (S1 no mergeado, `hasattr(Shell,"_exec_scp")` guard): local↔remoto y múltiples hosts persisten idénticos + determinismo. Suite 617→620 (+3). PR #38.**
