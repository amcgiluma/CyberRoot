...
- `[EN CURSO][P2]` (21/09) **S1 — Smough 16:00 · `feat/sandbox-2026-09-21` · `stat` como lector del testigo (`03:14`/`512` con ojos — ideas Havel 18/09+20/09)** — Handler `stat FICHERO` en `src/core/sandbox/commands/`: `stat /tmp/volcado-custodia.csv` → `Modify: 03:14:00` + `Size: 512` si rescate, exit 1 `cannot stat … No such file` si caducado. Boon `c.stat` hallazgo prereq `c.ls` en `src/data/curriculum.json` + glosa `help.stat` en `textos.json`. AC: `stat` exit 0/1 según variante; `stat` NO entra en las DEFAULT_CH5* allowlists (fuera de encargo → 127 honesto); gate **24→25 conceptos**/31 quests; suite +2–3. NO toca `session.py`, `postmortem.py`, `web/`.
