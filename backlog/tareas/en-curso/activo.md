...
- `[HECHO][P3]` (09/09) **T1 — Verificar circuito ch4 en vivo + dato del límite de pipes** — Seath (`feat/meta-ui-2026-09-09`): verificación del circuito COMPLETO (`generate(42,4)` → `cat /etc/hosts` 2-3 hosts → `scp troncal-01:…` multi-host → roundtrip `GameState` idéntico) + documentar el rechazo EXACTO de la pipeline de 4 eslabones (🧭25) como dato para la decisión de diseño de Gwyn (ampliar a 3 pipes vs enseñar a encadenar — 🪨 Havel 09/09 [P1]). AC: nota de verificación completa, suite +0 honesto o delta declarado, cero toques fuera de `docs/`+`src/tests/core/state/`. Origen: dirección de Gwyn (verificación) + 🪨 Havel 09/09 (pipes [P1] — hoy solo dato, decisión mañana). — **PR #41**
