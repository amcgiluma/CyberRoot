...
- `[HECHO][P2]` (25/09) **O1 — Ornstein 13:00 · `feat/engine-2026-09-25` · `grep -c` + factura frugal del e2** — ✅ PR #81 (Ornstein) — `_run_grep -c` (stdout `N\n`, exit GNU 0/1/2, combinable `-c/-i/-v`, stdin y ficheros), `postmortem.auditor.grep_c_count` vía `_extract_greps` (último grep exit 0 fue `-c censo` → factura frugal, hermana sin pisar huellas), `story.ch5.e2.hint_2` con factura frugal, bundle 50 fresco (488.3 KiB), tests 837 (818+19) — ✅ VERDE listo para merge primero (engine). — plan `../planes/2026/09/25.md` §O1.
- `[HECHO][P3]` (25/09) **S1 — Smough 16:00 · `feat/sandbox-2026-09-25` · 🧭47: contraste stock de Gris + lecturas intercaladas** — ✅ PR #82 (Smough) — corpus `--stock-gris` (histograma 6 huellas 100% determinista, N=8 azul 90.0%≥3 / rojo 90.0%≤-3 en 20 seeds, 99.6% en 500, stock Gris 0% estático, lectura intercalada delta 0.0% pct — medido honesto §8.6) + README harness 25/09 — ✅ VERDE listo para merge segundo (harness puro, +0 tests, `pytest 818` verde, `postmortem.py`/`shell.py`/`src/data/`/bundle intactos). — plan `../planes/2026/09/25.md` §S1.
- `[HECHO][P2]` (25/09) **T1 — Seath 19:00 · `feat/meta-ui-2026-09-25` · 6º estado `grep censo` en la lente del intruso** — ✅ PR #83 (Seath) — `#custodia-intruso` 6º estado `grep-verde` (`⌕` `#2ecc71`, jerarquía HUP/-9 > grep-verde > verde, PERSISTE hasta restartSameSeed, fuera cap5 oculto, caps 1-4 sin ensuciar) — ✅ VERDE listo para merge tercero (web puro, +0 tests, `node --check` OK, `CUSTODIA/TRONCAL_STATIC` byte-idénticas). — plan `../planes/2026/09/25.md` §T1.
