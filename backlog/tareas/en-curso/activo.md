...
- `[HECHO][P2]` (08/09) **S1 — Handler `scp` Fase B (red pieza 2, primera pieza del cap. 4)** — Smough (`feat/sandbox-2026-09-08`): copia entre FS del stack de conexión (`scp [user@]host:ruta destino`); host no descubierto → rechazo que NOMBRA qué falta y dónde (`/etc/hosts`), exit 1, ruido 0. AC: 5 tests (copia ok + user@, rechazo no-descubierto, ruta remota inexistente GNU-honesta, destino inválido ×2, roundtrip), 617→622 (+5). PR #37 pendiente (rama `feat/sandbox-2026-09-08`). Origen: red simulada P2 (31/08) — ELEGIDA HOY (solo handler; quests `ch4` mañana).
