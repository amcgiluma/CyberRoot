...
## [REVISIÓN SIN APLICACIÓN] (18/09) — por Gwyn
- Agente/job afectado: ninguno (sin propuesta nueva en `pendiente/propuestas.md` — última entrada: Gwyndolin identidad git 07/09, aplicada).
- Qué se cambió (del prompt): nada — sin propuestas que aprobar.
- Qué se mejoró / por qué: el incidente de ESTA noche valida el guard de autoría aplicado el 07/09: mi primer merge (#64) salió contaminado (mi worktree de ensayo estaba sobre la MISMA rama `feat/engine-2026-09-18` y el commit del ensayo entró en ella; el merge lo trajo a main). Detectado ANTES de pushear por el guard `%an` + inspección del grafo (el commit `a35dee5 ensayo merge O1+S1` firmado "Gwyn" pero con contenido mixto llegó a aparecer como ancestro de main durante décimas de segundo). Main reconstruido desde 4bf421a + merges limpios; la rama de Ornstein restaurada a 01aa9de. NADA contaminado llegó a origin/main (verificado `git log origin/main..HEAD` + SHA de GitHub post-push). Lección operativa registrada en worklog §23:00: NUNCA hace `merge` de las ramas del día sobre una rama de FEATURA que es un worktree compartido con otros turnos — ensayo siempre en worktree desechable con rama PROPIA creada para el ensayo (`gwyn-ensayo-<fecha>`), no sobre la rama del ejecutor. Cambio de PROCESO, no de prompt (mi prompt ya lo trae implícito vía regla de ensayo); anoto la precisión para la próxima pasada de doc si Artorias la pide.

## [REVISIÓN SIN APLICACIÓN] (20/09) — por Gwyn
- Agente/job afectado: ninguno (sin propuesta nueva en `pendiente/propuestas.md`).
- Qué se cambió (del prompt): nada — sin propuestas que aprobar.
- Qué se mejoró / por qué: el protocolo nocturno saturó los 3 incidentes potenciales de esta noche sin daño: (1) resolutor de huellas probado PRIMERO en worktree de ensayo — un bug de orden (assert sobre disco antes de escribir) saltó en el ensayo, nunca en main; (2) el ensayo detectó con antelación exacta los mismos 2 conflictos (activo.md + worklog) que después aparecieron en main — resolución idéntica, cero sorpresa; (3) regen canónico del bundle como paso canónico (773+1 stale → 774 passed). Las lecciones acumuladas (08/09 assertions de contenido, 18/09 worktree desechable) están rindiendo: día sin [BUG] nuevo pese a merge con huellas.
- Nota para mañana: Artorias/prompt ya exige declarar delta exacto — el off-by-1 de #68 (+4 vs +5) es la primera repetición del patrón «delta declarado ≠ real». Si un PR del 21/09 repite el off-by-1, abro propuesta [NUEVA] de aritmética obligatoria en el prompt de ejecutores.

## [APLICADA] (22/09) — por Gwyn
