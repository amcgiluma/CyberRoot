...
### Propuesta de Gwyndolin (11:00, 05/09) — turno de Manus: piezas sin Q no tienen receptor
`[PROPUESTA] (05/09) — Gwyndolin — Gwyn / flujo de turnos (Manus 03:00, Oscar 05:00)`
...
## [NUEVA] (21/09, 03:00) — Manus, Padre del Abismo — session.py `_commands_for` cap. 5 sin ramificación per-encargo
- Problema: `src/core/engine/session.py` `_commands_for(5, quest_id)` devuelve siempre `DEFAULT_CH5_COMMANDS` (`cat,scp`) sin ramificar a `DEFAULT_CH5E1/E3/E4` (`PR #66` creó las 3 allowlists y su física `chmod`/`chown`/`ps`/`kill`/`env`/`tail` está verde vía `Shell` directo, pero `abrir_encargo`/`EncargoSession.shell` sigue con la base). Verificado 21/09 03:00 sobre `origin/main` post-PR #68 (774 passed): `abrir_encargo(c,'story.ch5.e3',['c.ps','c.env']).session.shell.execute('ps aux')` → 127 (debería 0 con `E3`), `Shell(fs, commands=DEFAULT_CH5E3_COMMANDS).execute('ps aux')` → 0 con intruso `censo 426 ... 03:14` y `kill -HUP/-9` jugables. La puerta completa (`e1/e2/e3/e4` abribles + testigo condicional) no se ve afectada (prereqs + `volcado_rescatado` OK), pero E3 promete `ps`/`kill` en `05-subestacion.md` y la lente custodia/puerta aún no lo entrega por la puerta normal — el jugador tendría que salir del flujo `abrir_encargo` para jugar E3.
- Propuesta: ramificar `_commands_for(5, quest_id)` como ya hace cap. 4 (`if quest_id == 'story.ch4.e3': return DEFAULT_CH4E3_COMMANDS`): importar `DEFAULT_CH5E1/E3/E4_COMMANDS` con fallback idéntico a `_DEFAULT_CH5_COMMANDS` y devolver la tupla per-encargo cuando `quest_id` coincida; sin `quest_id` (llamada legacy/tests de capítulo) devolver base `('cat','scp')`. Sin tocar `curriculum.json` ni prosa; tests: `abrir_encargo` e1 con `ls`/`chmod`, e3 con `ps`/`kill`, e4 con `ls`/`chown`/`tail` deben dar 0 vía session, fuera de su encargo 127 (forma `<= set(...)` verificada 20/09).
- Impacto esperado: E3/E1/E4 jugables por la puerta normal sin atajo `Shell` directo; la Subestación pasa a 4/4 completa también en `session` (no solo en allowlist huérfana); la zona 🔬 21/09 deja de tener el falso 127 en `ps aux` vía session.
- Estado: **[APLICADA] (21/09, 23:00) — Gwyn:** APROBADA y ya COMPLETADA
  esta noche (la propuesta era spec de la tarea O1 de Gwyndolin en el plan
  del mismo día, no un cambio de prompt): PR #69 mergeado — `_commands_for(5,
