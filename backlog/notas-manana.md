...
### 🧭 Oscar — dirección 05:00 (18/09, MODO B — zona 🔬 testigo condicional Subestación + lente web COMPLETA, save limpio)

**Veredicto de experiencia:** APTO — el camino del novato sigue apto de principio a fin. La zona 🔬 18/09 se ejecutó COMPLETA desde save limpio (MODO B, `build_chapter5_fs(volcado_rescatado)` + `generate(volcado_rescatado)` + `web/app.js` tooltip/lente) y responde a la pregunta de sabor de Gwyn: ¿el testigo ausente se siente vacío (bug) o ausencia elocuente (drama)? → drama. La web acompaña sin anticipar.

**Qué se ha jugado (save limpio, sin atajos):**
- Prioridad 1 cap. 5 «Subestación»: el testigo condicional (PRs #62+#63): `build_chapter5_fs(Rng(42).fork("fs"), volcado_rescatado=True)` → `/tmp/volcado-custodia.csv` `TR-003|faro|troncal-01|512|EN_COLA` (66 bytes) + `Shell(fs, cwd="/tmp", commands=("cat","ps"))` `cat /tmp/volcado-custodia.csv` → exit 0 `TR-003`; variante `False` → NO existe → `cat` → exit 1 `No such file or directory` (trayectoria válida, `story.ch5.e2` briefing la documenta como pista). **Determinismo ×2 seeds byte-idéntico en AMBAS variantes** (`fork("ps-subestacion")` PID 420-434 estable, `to_dict`+`snapshot` idénticos). Proceso `intruso --vigilar-censo` USER `censo` START `03:14` patrulla igual en ambas (ausencia del papel no es ausencia de vigilancia). **Nota zona:** `session.py` aún NO expone cap. 5 (`SUPPORTED_CHAPTERS` {0,2,4} — `generate(42,5)` → `ValueError solo caps 0,2,3,4,6`) — probar por `build_chapter5_fs`+Shell directo, no por `abrir_encargo` (llega mañana, P1 de Gwyndolin). No-regresión 7 goldens byte-idénticos: `generate(42,0/2/3/4/6)` + `generate(42,6, volcado_rescatado=True/False)` (rescue≠caducado, rescue==rescue, caducado==caducado).
- Dato7 regresión (cap. 6): `generate(42,6, volcado_rescatado=True)` → `/srv/camara-faro/volcado-rescate.csv` + `Shell(DEFAULT_CH6_COMMANDS, cwd="/srv/camara-faro")` `join -t'|' -1 1 -2 1 -v 1 volcado-rescate.csv purgas.csv | grep TR-003` → exit 0 1 línea `TR-003|faro|troncal-01|512|EN_COLA` (sin `-v 1` → exit 1 vacío, golden exige `-v 1` honesto; con header `id` → 2 líneas si no `grep`); `generate(..., False)` → `join` → `join: volcado-rescate.csv: No such file or directory` exit 1 + `cat` igual; `generate(42,6)` por defecto sin volcado (no rompe dato4/dato6/e1).
- Prioridad 2 web: tooltip N/30 + lente rescate Faro (PR #61): `web/app.js` `renderTroncalTabla` con `TRONCAL_STATIC` byte-idéntica (3 ocurrencias) + `_tickTip` `<span title="… 30 ticks … rescate scp vs disolución rm … sin pulso"> · ticks del volcado: N/30</span>` (describe 30 ticks, sin `setInterval` — grep 0 hits `setInterval` en `web/app.js`, 🧭34) + `renderFaroTabla` suffix `TR-003 rescatado — volcado-rescate.csv` vía `_isFaroRescatePresent()` (lente pura, lee `get_history`/`get_csv`, sin ejecutar core). Verificado: `TRONCAL_STATIC` 3, `hideTroncalTabla` restart intacto, `node --check` OK; con save caducado lente NO aparece (falso positivo cazado).
- Smoke: `PYTHONPATH=src .venv/bin/python -m pytest src/ -o addopts= -q` → **749 passed / 0 failed** (gate 24/31, bundle 48 ficheros 441.4 KiB, `test_chapter5_fs` 8/8 `test_ch5_e2_testigo` 2/2, guardián verde), `SUPPORTED_CHAPTERS` {0,2,4} sin 5 esperado.

**Propuestas de dirección (informo, no decido — Gwyn valida):**
1. **🧭37 — CERRADO (verificación positiva 18/09):** el testigo condicional resuelve la pregunta de sabor de Gwyn — la ausencia `No such file` no se siente vacío/bug del FS sino **ausencia elocuente**: el briefing de `story.ch5.e2` la nombra como pista («si lo subiste, la prueba viaja; si lo borraste o tardaste, la hora amanece sin papel») y el `ps aux` con `intruso 03:14` patrulla igual en ambas variantes dando marco de vigilancia (no es FS roto, es papel perdido bajo vigilancia). Con `join -v 1 | grep TR-003` del Faro ya existía la segunda sombra del mismo gesto; que el cap. 5 lo repita con `cat`+`ps` lo hace trilogía. **No proponer huellas nuevas:** la geografía condicional ya pesa por contenido (`volcado-custodia.csv` presente/ausente), no por mecánica nueva. El interludio de Manus 03:00 («El papel en custodia») ya da prosa diegética al fichero. CERRADO.
2. **🧭38 — CERRADO (verificación positiva 18/09):** la lente web resuelve la segunda pregunta de sabor (¿acompaña o anticipa?) — **acompaña**: `N/30` es contador estático renderizado sin pulso (no `setInterval`, solo `title=` nativo que describe rescate `scp` vs disolución `rm`), y `TR-003 rescatado` solo aparece tras rescate (con save caducado NO aparece, falso positivo cazado). No ejecuta core (lente pura). `TRONCAL_STATIC` intacta 3 ocurrencias, `hideTroncalTabla` restart limpia el badge fantasma. **No tocar:** la lente no spoilea, no miente, no crece. CERRADO.
3. **Observación P1 para Gwyndolin (no [BUG], ya en plan):** `SUPPORTED_CHAPTERS` sin 5 es honesto hoy (la zona lo advierte: «probar por `generate`+Shell directo, no por `abrir_encargo`»). No es bug del camino: el FS condicional ya es jugable vía generator y la quest `ch5.e2` ya existe (`c.cat`+`c.scp`, sin concepto nuevo). La tarea P1 de mañana (cablear `session.py` ch5 con `_commands_for(5)` + flag `volcado_rescatado` del save/post-mortem e3) es el último paso para que el cap. 5 sea jugable por la puerta normal. No añadir comandos: `cat`+`ps`+`scp` ya es el repertorio.
4. **🧭24 — PERSISTE con matiz P3 (mantener pre-puebla):** `abrir_encargo`/`new_session` ch4 sigue pre-poblando `shell.hosts` con `faro`+`troncal-01/02` — `scp` sin `cat` ya no rechaza. Briefing documenta rechazo didáctico redundante. Gwyn decidió mantener (12/09); confirmo sin reproche (749/0). Si playtest confunde, reescribir briefing, no código.
...
**Saldo para Gwyn:** 🧭20/21/22/23 cerradas; 🧭24 cerrada con matiz P3; 🧭25/26/27 en recámara; 🧭28 cerrada; **🧭29/30 CERRADOS**; **🧭31/32/33 CERRADOS**; **🧭34/35 CERRADOS**; **🧭36 CERRADA**; **🧭37 CERRADO** (testigo condicional — ausencia elocuente, pregunta de sabor resuelta: drama no bug); **🧭38 CERRADO** (lente web acompaña, no anticipa). Sin [BUG] nuevo; `SUPPORTED_CHAPTERS` sin 5 es observación P1 planificada, no bug.

### 🧭 Oscar — dirección 05:00 (17/09, MODO B — zona 🔬 cadena e3→dato7 COMPLETA, save limpio) [ARCHIVADO]
