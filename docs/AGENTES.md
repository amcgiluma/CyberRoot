...
|---|---|---|---|---|
| 03:00 | **Manus** (historiador) | muse-spark-1.2 | Escribe la HISTORIA del día desde el plot. Prosa humanizada. | `backlog/historia/<fecha>.md` |
| 05:00 | **Oscar de Astora** (guardián de la experiencia) | muse-spark-1.2 | JUEGA como jugador de verdad: run de referencia desde save limpio + perspectiva de veterano (20+h). Comprueba la EXPERIENCIA global del juego y que el camino sea apto. Mantiene vivo `docs/ESTADO-JUGADOR.md` y deja NOTAS DE DIRECCIÓN a Gwyn (no decide, informa). | `docs/ESTADO-JUGADOR.md` |
| 07:00 | **Havel** (vidente-creativo) | muse-spark-1.2 | JUEGA el juego + GENERA ideas nuevas (capítulos/mecánicas/comandos) + anota bugs. NO lleva la crítica de diseño (eso es de Artorias/Gwyn). | `backlog/tareas/pendiente/abierto.md` |
...
> 🧠 **Modelo (decisión de Juanma, 04/09):** `muse-spark-1.2` = `opencode-go/muse-spark-1.2-contributor` (Manus/Oscar/Havel/Vigilante); `muse-spark-1.2` = `opencode-go/muse-spark-1.2-contributor` (Ornstein/Smough/Seath/Artorias — 1.3 peta en prompts largos, verificado 04/09)
