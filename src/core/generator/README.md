...
## v0.8 — FS de la Subestación cap. 5 «El asalto» — testigo condicional (O1, 17/09)

`chapter5.py` NUEVO (hoja leaf): `build_chapter5_fs(fs_rng, volcado_rescatado)` monta el FS de la Subestación con proceso `intruso --vigilar-censo` USER `censo` START `03:14` (firma horaria del volcado, misma que el `START 03:14` de `ps` en el troncal) + testigo `/tmp/volcado-custodia.csv` condicional: si `volcado_rescatado=True` contiene `TR-003|faro|troncal-01|512|EN_COLA` (misma línea que `volcado-rescate.csv` del Faro y `TRONCAL_CONTENT`), si caducado NO existe (`cat`→`No such file` — ausencia honesta como detector). Determinismo vía `fork("ps-subestacion")` → pid `420+below(15)` byte-idéntico; fallback estático. `CUSTODIA_PATH/CONTENT`, `INTRUSO_*`, `CHAPTER5_ENVIRONMENT`, `CANON_STEPS_RAW_CH5/_CADUCADO` exportados. `generator.py` añade `_generate_cap5(seed, variant, curriculum, contract_id, volcado_rescatado=False)` misma firma que dato7 (cap5 fuera de `SUPPORTED_CHAPTERS` HOY — sesión para mañana, NO toca `session.py`) y rama `validate_incursion` para `chapter==5` (intruso 03:14 + custodia). 8 tests `test_chapter5_fs.py` (determinismo ×2 seeds rescate/caducado, ps aux 03:14, No such file, 7 goldens byte-idénticos incl dato7 rescue/absence). Suite 739→747 (+8, bundle stale pendiente Gwyn por regla 12/09). `postmortem.*`, `curriculum.json`, `web/` intocados.
