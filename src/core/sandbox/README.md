...
> **Estado (13/09, Smough):** comandos del cap. 0 (`ls -a`/`-l` GNU, `cd`, `cat`,
> `cp`) + cap. 2 (`grep`, `wc`) y tubería `cmd1 | cmd2` + cap. 3 (`ps`, `env`
> — familia procesos; `sudo` GANADO + `kill` Señales v0) + familia conteo
> (`head`/`tail`/`sort`/`uniq`/`cut`, cap. 6 — lectura frugal) + **red cap.4 pieza1** (`ssh` básico + host-key + stack `exit`/`logout`, hosts como FS simultáneos) + **red cap.4 pieza2** (`scp` copia entre FS del stack, descubrimiento por `cat /etc/hosts`, rechazo que nombra, ruido 3) + **12/09** 127 didáctico `join` nombra Faro (`Try 'join --help' — tables cross there (chapter 6).`, solo join fuera de cap.6) + **13/09** quest `story.ch6.dato6` «La segunda purga» (coma-trampa PR-0092, `join | grep 000483` + variante `cut -f3 | grep`, 000 vs 000483 lección, 5 tests, bundle regen 24/29) + **14/09 S2** dato6 variante requirement (`cut -d'|' -f3 | grep 000483` ambas válidas) + hint_2 trampa `grep 000` vs `grep 000483` (7 tests, gate 24/29 intacto, bundle 402.4 KiB) + **15/09 S1** `rm` + `DEFAULT_CH4E3_COMMANDS` (14, ch4 base 13 intacta) + detector `postmortem.volcado.*` + `story.ch4.e3` (9 tests nuevos, gate 24/30, bundle 418.2 KiB) + **18/09 S1** `DEFAULT_CH5_COMMANDS` (`cat`+`scp`, allowlist nova ch5) + detector `postmortem.auditor.custodia` «custodia leída en casa: el testigo viajó» (4 tests, 753 passed, bundle 48/443.3 KiB) + **21/09 S1** `stat` como lector del testigo (`Modify: 03:14:00` + `Size: 512`, `c.stat` hallazgo prereq `c.ls`, 4 tests, gate 24→25, bundle 50/460.2 KiB) + **19/09 S1** Subestación allowlists e1/e3/e4 (`DEFAULT_CH5E1/E3/E4_COMMANDS` pattern CH4E3, base cat+scp intacta, frontera 127, 9 tests + golden kill -HUP/-9, suite 768+1 stale honesto).
...
| `shell.py` | Sesión serializable: parser `shlex` POSIX, registro de specs, cwd/tick/historial simulados, `DEFAULT_CAP0_COMMANDS` / `DEFAULT_CH2_COMMANDS` (+grep,wc) / `DEFAULT_CH3_COMMANDS` (+ps,env,sudo,kill S1 02/09) / `DEFAULT_CH6_COMMANDS` (+head,tail,sort,uniq,cut), tubería `cmd1 | cmd2` (stdout→stdin), rechazo didáctico. **S1, 03/09** 🧭14b: `read_marks` + bus `event.credential.read` + gate LECTURA `_exec_sudo` · **S1, 06/09** (red): `hosts`/`host_stack`/`known_hosts`/`pending_ssh`/`ssh_decisions`, `ssh [user@]host` + host-key OpenSSH determinista + `exit`/`logout` stack, serializable ida-vuelta · **S1, 07/09** `cat /etc/hosts` descubre hostnames no-locales en `hosts` (stub `FileSystem` vacío, solo lectura; `_parse_hosts_content`/`_note_hosts_discovery`) · **S1, 08/09** (red pieza2): `scp [user@]host:path path` copia entre FS del stack, host no descubierto → rechazo didáctico `host 'X' no descubierto — léelo en /etc/hosts` exit 1 ruido 0, GNU-honesto para ruta inexistente/destino inválido, `scp:3` |
| `commands/red.py` | **S1, 06/09** (red cap.4 pieza1): `ssh` básico + `exit`/`logout` des-apila, huellas `SHA256:` deterministas, prompt `The authenticity of host…` + `Are you sure…` (OpenSSH), ruido Red `ssh:2` · **S1, 08/09** (red pieza2): `scp [user@]host:path` copia entre FS (`_exec_scp` wrapper del shell), `SCP_SPEC` concepts `scp`, ruido `scp:3`, placeholder `SCP_NAME` |
| `commands/navigation.py` | `ls` (**S2, 06/09**: `-a`/`-l`/`-la` GNU-honesto, dotfiles ocultos sin `-a`, long format v0 con perms/size/mtime), `cd` (builtin: valida→normaliza) |
...
| `commands/conteo.py` | Familia conteo (**S1, 05/09**: `sort -k`/`-t`/`-n` lectura VERTICAL — `sort -t'|' -k12 -n` ordena la Lista por puntuación; **S2, 01/09**: `head`/`tail`/`sort`/`uniq`; **S2, 07/09**: `sort -k0`/`sort -t ab` GNU-honesto `Try 'sort --help'` auditado) `head`/`tail` (`-n N` default 10), `sort` (`-u`/`-n`/`-t SEP`/`-k KEYDEF`/`-r`, delim `|` incluido, fallback vacío sin crashear), `uniq` (`-c` ancho 7). «Lectura frugal»: leen menos que un `cat` entero. GNU honesto contrastado (multi-char tab, `-k0`, sin `-k` byte-idéntico). Cap. 6. |
| `commands/permisos.py` | `chmod`/`chown` (**S1, 19/09** — Subestación defensa: `chmod MODE FILE...` octal 600+alias +x, `chown OWNER[:GROUP] FILE...`, muta mode/owner/group+mtime, GNU-honesto exit 1, ruido 1, frontera 127) |
...
| `commands/stat.py` | `stat` (S1, 21/09 — T3): `stat FICHERO` lector del testigo `03:14`/`512` (Modify: 03:14:00 + Size: 512 si rescate, `cannot stat` exit 1 si caducado), hallazgo `c.stat` prereq `c.ls`, ruido 1, NO en allowlists CH5 (127 honesto) |
| `noise.py` | Perfil ⚠️ v1 `cd:0, ls:1, cat:1, cp:3, grep:2, wc:1, ps:1, env:1, sudo:3, head:1, tail:1, sort:2, uniq:1, kill:2, cut:1, ssh:2, scp:3, rm:2, chmod:1, chown:1` + eventos `common.events.Event` REALES; `exit`/`logout`:0 |
...
| `PLAN-2026-09-06.md` | Plan S1+S2 del turno 06/09 (ssh host-key + ls -a/-l, red cap.4 pieza1) |
| `PLAN-2026-09-08.md` | Plan S1 del turno 08/09 (scp Fase B — copia entre FS del stack, host no descubierto → rechazo que nombra, GNU-honesto) |
