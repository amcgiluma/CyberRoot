...
def test_todas_las_specs_estan_disponibles_para_el_set() -> None:
    ...
    01/09; cut, S1 04/09 — la Lista es tabla cortable) + stat (S1 21/09). `sudo` NO es una spec: es un wrapper del shell."""
    assert {s.name for s in SPECS_ALL} == {
        "ls", "cd", "cat", "cp", "grep", "wc", "ps", "env",
        "head", "tail", "sort", "uniq", "kill", "cut", "rm", "chmod", "chown", "stat",
    }
